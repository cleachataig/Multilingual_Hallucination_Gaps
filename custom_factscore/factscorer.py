...
from custom_factscore.abstain_detection import is_response_abstained
from custom_factscore.atomic_facts import AtomicFactGenerator
from custom_factscore.clm import CLM
from custom_factscore.npm import NPM
from custom_factscore.retrieval import DocDB, Retrieval
...
class FactScorer(object):
    ...
    def __init__(self,
                 ...
                 batch_size=256):
        assert model_name in ["retrieval+llama", "retrieval+llama+npm", "npm", "retrieval+mistral", "retrieval+mistral+npm"]
        ...
        if 'mistral' in model_name:
            cache_file=os.path.join(self.cache_dir, "Mistral.pkl")
        elif 'llama' in model_name:
            cache_file=os.path.join(self.cache_dir, "Llama.pkl")
        else:
            cache_file=os.path.join(self.cache_dir, "Model.pkl")

        self.lm = CLM(model_dir, cache_dir, device, cache_file)
        self.af_generator = AtomicFactGenerator(model=self.lm,
                                                demon_dir=os.path.join(self.data_dir, "demos")
                                                )
        self.true_token_id, self.false_token_id=self.lm.true_token_id, self.lm.false_token_id
    ...
    def _get_score(self, topic, atomic_facts, knowledge_source, cost_estimate=None):
        decisions = []
        total_words = 0
        ...
        for atom in atomic_facts:
            ...
            if self.lm:                
                ...
                if type(output)==np.ndarray:
                    true_score = output[0, self.true_token_id] # token id for "True"
                    false_score = output[0, self.false_token_id] # token id for "False"
